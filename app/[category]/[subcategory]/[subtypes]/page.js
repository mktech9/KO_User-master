...
export async function generateMetadata({ params }) {
  ...
  let cate = decodeURIComponent(params?.category?.replace(/-/g, " ")),
    subCate = decodeURIComponent(params?.subtypes?.replace(/-/g, " "));
  const [category, subCategory] = await Promise.all([
    await Category.findOne({ name: cate }),
    await SubType.findOne({ name: subCate }),
  ]);

  return {
    title: subCategory?.title,
    description: subCategory?.subText,
    robots: {
      index: true,
      googleBot: {
        index: true,
    ...
    openGraph: {
      title: subCategory?.title,
      description: subCategory?.subText,
      url: `${configs?.url}/${params?.category}/${params?.subcategory}/${params?.subtypes}`,
...
const Page = async ({ params, searchParams }) => {
  ...
  let cate = decodeURIComponent(params?.category?.replace(/-/g, " ")),
    subCate = decodeURIComponent(params?.subcategory?.replace(/-/g, " ")),
    subType = decodeURIComponent(params?.subtypes?.replace(/-/g, " "));
  ...
  if (subCate) {
    parameters.subCategory = [subCate];
  ...
  if (parameters.isSingleCategory) {
    titleData = await getTitle(subCate, subType, category);
  }
